...
export class WsReceiver extends Writable {
    ...
    private readInfo() {
        ...
        if ((buf[0] & 0x30) !== 0x00) {
            this._loop = false;
            return MakeError(RangeError, 'RSV2 and RSV3 must be clear', 1002);
        }
        ...
        if (this._opcode === 0x00) {
            ...
            if (!this._fragmented) {
                this._loop = false;
                return MakeError(RangeError, 'invalid opcode 0', 1002);
            }
            ...
            if (this._fragmented) {
                this._loop = false;
                return MakeError(RangeError, `invalid opcode ${this._opcode}`, 1002);
            }
            ...
            if (!this._fin) {
                this._loop = false;
                return MakeError(RangeError, 'FIN must be set', 1002);
            }
            ...
            if (this._payloadLength > 0x7d) {
                this._loop = false;
                return MakeError(RangeError, `invalid payload length ${this._payloadLength}`, 1002);
            }
        }
    ...
    haveLength() {
        if (this._payloadLength && this._opcode < 0x08) {
            ...
            if (this._totalPayloadLength > this._maxPayload && this._maxPayload > 0) {
                this._loop = false;
                return MakeError(RangeError, 'Max payload size exceeded', 1009);
            }
        }
    ...
    private handleControlMessage(data: Buffer) {
        if (this._opcode === 0x08) {
            ...
            if (data.length === 0) {
                ...
                return MakeError(RangeError, 'invalid payload length 1', 1002);
            }
                ...
                if (!IsValidStatusCode(code)) {
                    return MakeError(RangeError, `invalid status code ${code}`, 1002);
                }
